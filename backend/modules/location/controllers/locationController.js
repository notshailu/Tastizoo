  ...
  try {
    ...
    try {
      ...
      if (!response) {
        try {
          logger.warn(
            "All OLA Maps authentication methods failed, using fallback service",
            {
          ...
          try {
            ...
            if (!formattedAddress) {
              ...
              if (fallbackData.locality || fallbackData.city)
                parts.push(fallbackData.locality || fallbackData.city);
              ...
              results: [
                {
                  ...
                  address_components: {
                    ...
                    state:
                      ...
                      fallbackData.administrativeArea,
      ...
      if (response && response.data) {
        ...
        if (
          olaData.results &&
          Array.isArray(olaData.results) &&
          olaData.results.length > 0
        ) {
          ...
          if (
            ...
            Array.isArray(firstResult.address_components)
          ) {
            let area = "";
            let city = "";
            let state = "";
            ...
            if (!area && formattedAddress) {
              const parts = formattedAddress
                ...
                .map((p) => p.trim())
                .filter((p) => p.length > 0);
              logger.info("Extracting area from formatted_address", {
                parts,
                city,
                state,
              });
              ...
              if (parts.length >= 3) {
                ...
                if (
                  potentialArea &&
                  potentialArea.toLowerCase() !==
                    (cityPart || "").toLowerCase() &&
                  potentialArea.toLowerCase() !==
                  ...
                  area = potentialArea;
                  logger.info(
                    "✅ Extracted area from formatted_address (3+ parts):",
                    area,
                  );
                  ...
                  if (
                    cityPart &&
                    (!city || cityPart.toLowerCase() !== city.toLowerCase())
                  ) {
                    city = cityPart;
                  }
                  if (
                    statePart &&
                    (!state || statePart.toLowerCase() !== state.toLowerCase())
                  ) {
                    state = statePart;
                  }
                }
                ...
                if (
                  ...
                  area = firstPart;
                  logger.info(
                    "✅ Extracted area from formatted_address (2 parts):",
                    area,
                  );
                }
              }
